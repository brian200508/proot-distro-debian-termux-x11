...
Usage: ./install-cli.sh [options]
  ...
  -y, --yes                  Accept all defaults, skip every confirmation
                              ...
                              Implies --autostart-debian --bashrc-theme and
                              username "debian" unless -u is also given.
  -h, --help                 Show this help and exit.

Examples:
  ./install-cli.sh --yes
  ./install-cli.sh -u brian -p hunter2 --autostart-debian
EOF
}
...
echo "${C}${BOLD} with Fresh terminal editor in Termux"${W}
echo
}
...
function setup_bashrc_theme() {
    ...
    # has to go through `proot-distro login`, unlike the function above.
...
curl -Lf https://raw.githubusercontent.com/brian200508/debtx11/main/startpd-cli.sh -o ~/startpd.sh
